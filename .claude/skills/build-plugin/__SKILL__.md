...
44. **CSRF tokens from SSR initial state expire after page load.** Some SPAs (e.g., Tumblr's Redpop) embed a CSRF token in the SSR'd initial state (e.g., `<script type="application/json" id="___INITIAL_STATE___">` containing `csrfToken`), but this token expires shortly after the page loads. The SPA refreshes it by reading the `x-csrf` response header from each API call. Write operations using the stale initial token fail with 401 (not 403 — the error looks like an auth failure, not a CSRF rejection). The fix: before every write operation, make a lightweight GET request (e.g., `/api/v2/user/info`) to obtain a fresh CSRF from the response `x-csrf` header, and also update the cached CSRF from every API response. During Phase 2, test write operations immediately after page load AND after a delay — if writes work immediately but fail minutes later, the CSRF token is rotating
