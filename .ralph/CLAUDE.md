...
- **Self-contained containers — no host pollution.** Containers use `HOME=/tmp/worker` (container-local). All writes — `.opentabs`, `.claude`, node caches, Playwright profiles — stay inside the container and are discarded on exit. The only host mounts are the worktree (read-write for git commits) and the main `.git` directory (for worktree resolution). No `~/.opentabs`, `~/.claude`, or `~/.gitconfig` mounts.
