...
When building request bodies with optional fields, use `stripUndefined` to remove `undefined` keys instead of writing manual `if` checks. It keeps `null`, `0`, `false`, and empty string — only `undefined` is filtered out:
...
The SDK provides several fetch wrappers. All of them automatically include `credentials: 'include'` (so session cookies are sent) and apply a **30-second default timeout** via `AbortSignal.timeout()`. Use this table to pick the right one:
...
All the JSON and form helpers throw a `ToolError` on non-2xx responses, so you don't need manual status checks. If you need custom error handling, use `fetchFromPage` directly and inspect the `Response`.
...
When using `fetchFromPage` directly (instead of `fetchJSON` / `postJSON` which handle errors automatically), use `httpStatusToToolError` to convert HTTP status codes into the correct `ToolError` category:
...
`httpStatusToToolError` maps status codes to error categories: 401/403 → `auth`, 404 → `notFound`, 429 → `rateLimited` (with `Retry-After` header support), 400/422 → `validation`, and everything else → `internal`.
