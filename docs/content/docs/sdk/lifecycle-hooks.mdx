...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>When the adapter is being removed — on plugin update, plugin uninstall, or when the tab navigates away from a matching URL.</Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell className="font-medium">Re-injection</Table.Cell>
      <Table.Cell>`teardown()`</Table.Cell>
      <Table.Cell>After `onDeactivate()`, before the new adapter version is injected. Fires on plugin update (re-injection) and plugin uninstall.</Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
